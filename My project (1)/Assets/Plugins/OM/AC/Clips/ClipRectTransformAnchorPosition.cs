...
namespace OM.AC
{
    ...
    public class ClipRectTransformAnchorPosition : ACTweenClip<Vector3, RectTransform>
    {
        ...
        {
            if (state == ACEvaluateState.Finished)
            {
                SetValue(Vector3.LerpUnclamped(from, to, ease.Lerp(1)));
            }
            if (state != ACEvaluateState.Running) return;
            SetValue(Vector3.LerpUnclamped(from, to, ease.Lerp(normalizedClipTime)));
        }
    }
}