...
public class EventSchedule
{
    ...
    {
        m_EventHandler = new MultiDictionary<System.Type, BaseRegister>();
    ...
    public bool Check(System.Type type, BaseRegister handler)
    {
        if (handler == null)
        {
        ...
        return m_EventHandler.Contains(type, handler);
    }

    public void Subscribe(System.Type type, BaseRegister handler)
    {
        if (handler == null)
        {
            throw new System.Exception("Event handler is invalid.");
        ...
        if (!m_EventHandler.Contains(type))
        {
            m_EventHandler.Add(type, handler);
        }

    }

    public void Unsubscribe(System.Type type, BaseRegister handler)
    {
        if (handler == null)
        {
            throw new System.Exception("Event handler is invalid.");
        ...
        if (!m_EventHandler.Remove(type, handler))
        {
            throw new System.Exception(string.Format("Event '{0}' not exists specified handler.", type));
        }
    }
    ...
    private void HandleEvent<T>(ref T e) where T : IEvent
    {
        ...
        if (m_EventHandler.TryGetValue(e.GetType(), out range))
        {
            ...
            while (current != null && current != range.Terminal)
            {
                ...
                {
                    if (conditionRegister.Condition != null && conditionRegister.Condition is Func<T, bool> cond && cond(e))
                    {
                        current.Value.Invoke(e);
                    }
                    else
                    {
                        UnityEngine.Debug.LogError($"Event {type} not match condition");
                        current.Value.Invoke(e);
                    }
                }
                else if (e is IChannelEvent channelEvent && current.Value is ChannelRegister channelRegister)
                {
                    if (channelRegister.Channel == channelEvent.Channel)
                    {
                        current.Value.Invoke(e);
                    }
                    else
