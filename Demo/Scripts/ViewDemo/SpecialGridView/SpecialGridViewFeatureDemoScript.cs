...
namespace NFramework.Module.UI.ScrollView
{
    public class SpecialGridViewFeatureDemoScript : MonoBehaviour
    {
        ...
        if one row is make up with 3 subitems, then the GridView looks like:
            ...
            ...
        */
        View OnGetItemByIndex(LoopList listView, int rowIndex)
        {
            if (rowIndex < 0 )
            {
                return null;
        ...
        View NewFeatureItems(LoopList listView, int rowIndex)
        {
            if(rowIndex >= mFeatureArray.Length)
            {
                return null;
            ...
            View item = listView.NewListViewItem(mFeaturePrefabs[rowIndex]);  
        ...
        View NewMainItems(LoopList listView, int rowIndex)
        {            
            if(rowIndex < mFeatureArray.Length)
            {
                return null;
            ...
            View item = listView.NewListViewItem("ItemPrefab3");
            BaseHorizontalItemList itemScript = item.GetComponent<BaseHorizontalItemList>();
            if (item.IsInitHandlerCalled == false)
            {
                item.IsInitHandlerCalled = true;
